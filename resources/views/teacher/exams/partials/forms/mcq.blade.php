<!-- MCQ Modal Form -->
<div id="mcqQuestionForm" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        ...
        <form action="{{ route('teacher.exams.items.store', $exam->id) }}" method="POST">
            ...
            <div class="sticky top-0 bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6 rounded-t-xl">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold">Multiple Choice Question</h3>
                        <p class="text-indigo-100 text-sm mt-1">Create a question with multiple answer options</p>
                    </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div id="mcqOptionsContainer" class="space-y-3">
                        <div class="flex items-center space-x-2">
                            <input type="checkbox" name="options[0][correct]" value="1" class="w-5 h-5 text-indigo-600">
                            <input type="text" name="options[0][text]" placeholder="Option 1" required
                                class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        </div>
                        <div class="flex items-center space-x-2">
                            <input type="checkbox" name="options[1][correct]" value="1" class="w-5 h-5 text-indigo-600">
...
function addMcqOption() {
    ...
    optionDiv.innerHTML = `
        <input type="checkbox" name="options[${mcqOptionIndex}][correct]" value="1" class="w-5 h-5 text-indigo-600">
        <input type="text" name="options[${mcqOptionIndex}][text]" placeholder="Option ${mcqOptionIndex + 1}" required
            class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
